...
describe(createRequestHandler, () => {
  ...
  describe('loader requests', () => {
    it('returns loader data as JSON for a route with loader', async () => {
      ...
      // The loader no-store default is an exported `pageHeaders` rule, not runtime behavior.
    ...
    describe('Cache-Control resolution', () => {
      ...
      it('passes a loader-declared Cache-Control through verbatim', async () => {
        const handler = createLoaderHandler({}, async () =>
      ...
      it('applies a pageHeaders rule matched on the loader path', async () => {
        const handler = createLoaderHandler(
          {
            pageHeaders: [
              {
